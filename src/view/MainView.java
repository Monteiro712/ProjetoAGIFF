	...
	public void displayEscolha(Object opcaoEscolhida, String[] arrayOpcoes) {
		...
		while((opcaoEscolhida != null) && (opcaoEscolhida.toString().length()>0) && (opcaoEscolhida != "Sair")) {	
			...
			if((opcaoEscolhida == "Cadastrar Publicação") && (this.colecaoController.getListaColecoes().isEmpty()!=true)) {	
				...
				Object colecaoTitulo = JOptionPane.showInputDialog(null, "Escolha uma coleção para cadastrar sua publição.", "AgIFF" , JOptionPane.PLAIN_MESSAGE, null, arrayOpcoesDeColecoes, "Regular");
			...
			if(opcaoEscolhida == "Cadastrar uma Tag") {	
				...
				String descricao = JOptionPane.showInputDialog("Informe a descrição da Tag: ");
				if(nome.isBlank()||nome.isEmpty()) {
					JOptionPane.showMessageDialog(null, "Nome inválida. Tente novamente.", "AgIFF" , JOptionPane.PLAIN_MESSAGE);
				}
				else {
